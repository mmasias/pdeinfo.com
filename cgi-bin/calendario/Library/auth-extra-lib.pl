...
# Application Name: AUTH-EXTRA-LIB.PL
...
# Description:
# 
#    Provides a supporting set of routines to the AUTH-LIB.PL library
#    program.
#
# Main Procedures:
#
...
# Special Notes: 
#
#    This script relies on mail-lib.pl for sending
...
#       mentioned above, it is a support file for AUTH-LIB.PL. 
#        
...
# We keep the cosmetic HTML output in a separate file
...
# was submitted (which button was pressed) on each
# screen.
#
...
# If it is older than auth_session_length, delete it
        if (-M "$auth_session_dir/$file" > 
               $auth_session_length)
                {
                # READDIR is safe on CGI, so we unlink simply
                # clear taintmode with the following 2 lines
                $file =~ /(.*)/;
                $file = $1;
...
sub SearchUsers {
	...
	shift(@extra_fields);
	shift(@extra_fields);
	if ($auth_email =~ /^$extra_fields[$field_num]$/i) {
	    $user_match .= $username . "|";
	} # End of email match
    } # End of While
    close (USERFILE);
...
#
#   Output:
#      None.  It checks for the existence of a lock file
