...
else
{
	...
	$direccion_actual = @tmp[1];
	$telefono = @tmp[2];
	$correo_electronico = @tmp[3];
	$lugar_y_fecha_de_nacimiento = @tmp[4];
	$documento_de_identidad = @tmp[5];
	$primaria = @tmp[6];
	$secundaria = @tmp[7];
	$superior = @tmp[8];
	$otros_estudios =@tmp[9];
	$lengua_nativa = @tmp[10];
	$idioma_1 = @tmp[11];
	$idioma_2 = @tmp[12];
	$otro_idioma = @tmp[13];
	$cursos_y_seminarios = @tmp[14];
	$experiencia_laboral = @tmp[15];
	$software_que_maneja = @tmp[16];
	$hobbies = @tmp[17];
	$intereses_profesionales = @tmp[18];
